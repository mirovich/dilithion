 ...
 * Used by the main node to update mining templates without creating a
 * compile-time dependency on node-specific code.
 *
 * @param db         Reference to blockchain database
 * @param new_height The new chain tip height
 * @param is_reorg   True if this was a reorg, false for new block
 */
using ChainTipUpdateCallback = std::function<void(CBlockchainDB& db, int new_height, bool is_reorg)>;

/**
 * @brief Set the callback for chain tip updates
 *
 * Called by the main node at startup to register a handler for chain tip changes.
 *
